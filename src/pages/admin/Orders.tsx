...
export default function AdminOrders() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-xl">Pipeline de encomendas</h1>
          ...
          return (
            <div key={status} className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className={`text-[11px] px-2 py-0.5 rounded-full border ${statusColors[status]}`}>
                  {statusOrders.length}
                </span>
              </div>
                  ...
                  <Card key={order.id} className="bg-card border-border/50">
                    <CardContent className="p-4 space-y-3">
                      <div className="flex items-start justify-between">
                        <div>
                          <p className="font-medium text-sm">{order.clubName}</p>
                      ...
                      <div className="flex items-center gap-3 text-xs text-muted-foreground">
                        <span className="flex items-center gap-1">
                          <Package className="w-3 h-3" /> {order.quantity} un.
                        </span>
                        <span className="flex items-center gap-1">
                          <Calendar className="w-3 h-3" /> {order.deadline}
                        </span>
                      </div>
                      <p className="text-sm font-display tabular-nums">{order.total.toFixed(2)} €</p>
