...
import { clubs, orders, statusLabels, statusColors } from '@/data/mockData';
...
export default function AdminClients() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-xl">Gestão de clubes</h1>
          ...
          <Table>
            ...
            <TableBody>
              {filtered.map((c) => (
                ...
                >
                  <TableCell className="text-xl">{c.logo}</TableCell>
          ...
          {club && (
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 my-4">
                ...
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  <Phone className="w-3.5 h-3.5" /> {club.contact}
                </div>
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  <Mail className="w-3.5 h-3.5" /> {club.email}
                </div>
              </div>
                  ...
                  {clubOrders.map((o) => (
                    <div key={o.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                      ...
                      <div className="flex items-center gap-3">
                        <span className={`text-[11px] px-2 py-0.5 rounded-full border ${statusColors[o.status]}`}>
                          {statusLabels[o.status]}
                        </span>
                        <span className="font-display text-sm tabular-nums">{o.total.toFixed(2)} €</span>
                      </div>
                    </div>
                  ))}
                </div>
